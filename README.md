...
- **异构文档快速分类拆分**：适合合同、发票、发货单等不同类型文档混在同一个 PDF 或多个文件中的情况。这个模式只做页面分类、逻辑文档切分和文件保存，不提取字段。主表、附表、附件、明细页或版式不同的页面会分别拆成可独立上传的子类型，不会把“看起来相关”的页面强行合并。分类完成后可下载每个类型的 PDF，或下载全部分类 ZIP，再把每种类型单独上传到前两个模式中提取。

前两种 AI 模式都支持先分析字段方案、人工修改后再提取；也可以直接点击一键按钮自动完成分析和提取。异构模式按“快速识别类型 → 切分逻辑文档 → 保存分组文件 → 分别上传提取”执行，不会改变前两种模式的字段提取逻辑。

AI 不依赖项目内置的银行名称、票据版式或固定坐标规则。字段名称不一致时，会依据字段说明、页面上下文、表头和数值关系判断业务含义；无法可靠判断的字段或文档边界会保留证据并标记待复核。分类网关暂时不可用时，程序会按页面标题和版式证据做降级分组，并在结果页明确标注“按版式兜底”，不会静默把所有页面合成一个“未知文档”。
...
├── mixed_mode.py       # 旧版混合结果组装（保留兼容）
├── split_mode.py       # 异构文档快速分类、边界校正、PDF/ZIP 保存
...
统一明细、图片直传、三种提取模式、异构文档分类切分下载、校验和 Excel 导出流程。
